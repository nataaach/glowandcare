...
// --- ЗАВДАННЯ 11. Ініціалізація кешу ---
const cache = new NodeCache({ stdTTL: 60 }); // Кеш на 60 секунд

app.use(helmet()); // Приховує вразливі заголовки 
...
app.get('/', async (req, res) => {
    try {
        const cachedData = cache.get('main_page_data');
        if (cachedData) {
            return res.render('index', { products: cachedData.products, categories: cachedData.categories });
        }

        const products = await db.Product.findAll();
        const categories = await db.Category.findAll();

        cache.set('main_page_data', { products, categories }); // Зберігаємо в кеш
...
app.get('/api/products', async (req, res) => {
    try {
        const products = await db.Product.findAll();
        res.json(products); // Повертаємо саме JSON масив
    } catch (err) {
        logError(err);
...
app.post('/create-order', [
    ...
    if (!errors.isEmpty()) {
        // Повертаємо першу помилку зі списку (як у твоєму прикладі)
    ...
    try {
        ...
        // Тут буде твоя логіка збереження замовлення в базу даних (db.Order.create...)
        
        // Очищуємо кеш товарів, якщо замовлення впливає на їх кількість
...
app.post('/add-product', async (req, res) => {
    try {
        ...
        // --- ЗАВДАННЯ 11. Очищення кешу при зміні даних ---
        cache.del('main_page_data');
